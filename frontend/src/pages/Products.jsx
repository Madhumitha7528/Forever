          ...
          <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
            {
              productData.image.map((item,i) => (
                <img onClick={()=>setImage(item)} src={item} key={i} alt="" className='w-[24%] sm:w-full sm:mb-3 flex shrink-0 cursor-pointer' />
              ))
            }
          </div>
          ...
          <div className='flex flex-col gap-4 my-8'>
            ...
            <div className='flex gap-2'>
              {productData.sizes.map((item,i) => (
                <button onClick={()=>setSize(item)} className={`border py-2 px-4 bg-gray-100 ${item === size ? 'border-orange-500' : ''}`} key={i}>{item}</button>
              ))}
            </div>
          </div>
